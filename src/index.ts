...
const TOOLS: Tool[] = [
  {
    ...
    description:
      "Get system overview: OS name/version, hostname, uptime, hardware manufacturer, model, BIOS version, and motherboard info.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_cpu_info",
    description:
      "Get CPU details: name, physical/logical core count, max clock speed (MHz), current load (%), and temperature (°C). " +
  ...
  },
  {
    name: "get_gpu_info",
    description:
      "Get GPU details for all GPUs. NVIDIA: full telemetry via nvidia-smi " +
  ...
  },
  {
    name: "get_ram_info",
    description:
      "Get RAM usage: total/used/available (GB), usage (%), and DIMM slot details (capacity GB, speed MHz, manufacturer, type DDR4/DDR5). " +
  ...
  },
  {
    name: "get_fan_info",
    description:
      "Get fan speeds (RPM) for all system fans. " +
  ...
  },
  {
    name: "get_disk_info",
    description:
      "Get storage info: drive model, total size (GB), media type, interface (NVMe/SATA/USB), " +
  ...
  },
  {
    ...
    description:
      "Get network adapter info (IP addresses, MAC, DHCP, DNS, gateway) and real-time per-interface throughput (bytes sent/received per second).",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_all_system_info",
    description:
      "Get ALL system metrics in one call: overview + CPU + GPU + RAM + fans + disks + network. Best for a complete health snapshot.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_server_info",
    description:
      "Get MCP server information: detected platform, WSL status, feature support, and setup guidance.",
    inputSchema: { type: "object", properties: {} },
  },
];
...
const server = new Server(
  { name: "pc-system-info-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          wslNote:
            "When running inside WSL, the server automatically calls Windows powershell.exe to collect Windows-native metrics.",
        }, null, 2),
      }],
    };
  }
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `[pc-system-info-mcp] Ready | platform=${PLATFORM} wsl=${IS_WSL}\n`
  );
}
